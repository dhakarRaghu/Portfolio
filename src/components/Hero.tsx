...
import { Mail, ExternalLink, Code, Trophy } from 'lucide-react';
...
const Hero = () => {
  ...
  //   { label: 'GitHub Projects', value: '15+', icon: ExternalLink },
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                className="group relative px-6 py-3 bg-primary text-primary-foreground rounded-lg font-medium transition-all duration-300 hover:shadow-lg hover:shadow-primary/25 hover:scale-105 active:scale-95"
              >
              ...
              <a
                ...
                className="group relative px-6 py-3 bg-secondary/20 border border-secondary/50 text-secondary rounded-lg font-medium transition-all duration-300 hover:shadow-lg hover:shadow-secondary/25 hover:bg-secondary/30 hover:scale-105 active:scale-95"
              >
              ...
              <div className="flex flex-wrap gap-3">
                {socialLinks.map(({ name, url, icon, color }) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`group relative p-3 text-muted-foreground rounded-xl border border-border/50 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:border-transparent active:scale-95 ${color}`}
                    aria-label={name}
                  >
                    {icon}
                  </a>
          ...
          <div className="flex flex-col items-center space-y-8">
            ...
            {/* <div className="grid grid-cols-2 gap-4 w-full max-w-md">
              {stats.map(({ label, value, icon: Icon }, index) => (
                <div
                  key={label}
                  className="group relative p-4 bg-card/30 backdrop-blur-sm border border-border/50 rounded-lg transition-all duration-300 hover:border-primary/50 hover:scale-105 hover:shadow-lg hover:shadow-primary/30 active:bg-primary/10"
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="flex items-center space-x-2 mb-1">
                    <Icon className="h-4 w-4 text-primary group-hover:scale-125 transition-transform duration-300" />
